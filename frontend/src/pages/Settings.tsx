  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/80 dark:bg-slate-800/50 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <h2 className="text-sm font-medium text-gray-500 dark:text-slate-400 mb-4">테마</h2>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-gray-900 dark:text-white font-medium">{isDark ? '다크 모드' : '라이트 모드'}</p>
              <p className="text-xs text-gray-500 dark:text-slate-500">현재 {isDark ? '어두운' : '밝은'} 테마 사용 중</p>
            </div>
          </div>
          <button
            onClick={toggleTheme}
      ...
      <div className="bg-white/80 dark:bg-slate-800/50 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <h2 className="text-sm font-medium text-gray-500 dark:text-slate-400 mb-4">알림</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-slate-700 flex items-center justify-center">
                <svg className="w-5 h-5 text-gray-500 dark:text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                </svg>
              </div>
              <div>
                <p className="text-gray-900 dark:text-white font-medium">효과음</p>
                <p className="text-xs text-gray-500 dark:text-slate-500">위험도에 따른 알림음</p>
              </div>
            </div>
            <button
              onClick={handleSoundToggle}
              className={`relative w-12 h-6 rounded-full transition-colors ${notifications.sound ? 'bg-primary-600' : 'bg-gray-300 dark:bg-slate-600'}`}
            >
              <span
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-gray-900 dark:text-white font-medium">진동</p>
                <p className="text-xs text-gray-500 dark:text-slate-500">위험 감지 시 진동 알림</p>
              </div>
            </div>
            <button
              onClick={handleVibrationToggle}
              className={`relative w-12 h-6 rounded-full transition-colors ${notifications.vibration ? 'bg-primary-600' : 'bg-gray-300 dark:bg-slate-600'}`}
            >
              <span
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
          <p className="text-xs text-gray-500 dark:text-slate-500 mb-2">알림 테스트</p>
          <div className="flex gap-2">
            <button
              onClick={() => testNotification('GREEN')}
              className="flex-1 py-2 bg-green-500/20 text-green-600 dark:text-green-400 rounded-lg text-sm hover:bg-green-500/30"
            >
              안전
            </button>
            <button
              onClick={() => testNotification('YELLOW')}
              className="flex-1 py-2 bg-yellow-500/20 text-yellow-600 dark:text-yellow-400 rounded-lg text-sm hover:bg-yellow-500/30"
            >
              주의
            </button>
            <button
              onClick={() => testNotification('RED')}
              className="flex-1 py-2 bg-red-500/20 text-red-600 dark:text-red-400 rounded-lg text-sm hover:bg-red-500/30"
            >
              위험
            </button>
          </div>
      ...
      {/* About */}
      <div className="bg-white/80 dark:bg-slate-800/50 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <h2 className="text-sm font-medium text-gray-500 dark:text-slate-400 mb-3">정보</h2>
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-slate-500">버전</span>
            <span className="text-gray-700 dark:text-slate-300">1.0.0</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500 dark:text-slate-500">개발</span>
            <span className="text-gray-700 dark:text-slate-300">QR Guardian Team</span>
          </div>
        </div>
      </div>
